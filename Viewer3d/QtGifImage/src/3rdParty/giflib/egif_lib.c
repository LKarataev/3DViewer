...
int EGifPutScreenDesc(GifFileType *GifFile, const int Width, const int Height,
  ...
  if ((unsigned)InternalWrite(GifFile, (unsigned char *)write_version,
...
******************************************************************************/
int EGifPutCode(GifFileType *GifFile, const GifByteType *CodeBlock) {
  GifFilePrivateType *Private = (GifFilePrivateType *)GifFile->Private;
...
static int EGifBufferedOutput(GifFileType *GifFile, GifByteType *Buf, int c) {
  if (c == FLUSH_OUTPUT) {
    ...
    if (Buf[0] != 0 && (unsigned)InternalWrite(GifFile, Buf, Buf[0] + 1) !=
                           (unsigned)(Buf[0] + 1)) {
      GifFile->Error = E_GIF_ERR_WRITE_FAILED;
      return GIF_ERROR;
    }
